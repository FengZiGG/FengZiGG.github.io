...
NexT.utils = {
  ...
  wrapImageWithFancyBox : function() {
    document.querySelectorAll('.post-body :not(a) > img, .post-body > img')
        .forEach(element => {
          var $image = $(element);
          var imageLink = $image.attr('data-src') || $image.attr('src');
          var $imageWrapLink =
              $image
                  .wrap(`<a class="fancybox fancybox.image" href="${
                      imageLink}" itemscope itemtype="http://schema.org/ImageObject" itemprop="url"></a>`)
                  .parent('a');
          if ($image.is('.post-gallery img')) {
            ...
            $imageWrapLink.attr('data-fancybox', 'default')
                .attr('rel', 'default');
          }
    ...
    $('.fancybox')
        .fancybox({loop : true, helpers : {overlay : {locked : false}}});
  },
      ...
      button.addEventListener('click', event => {
        ...
        if (CONFIG.copycode.show_result) {
          target.querySelector('i').className =
              result ? 'fa fa-check fa-fw' : 'fa fa-times fa-fw';
        }
        ta.blur(); // For iOS
        target.blur();
        if (selected) {
  ...
  registerVideoIframe : function() {
    document.querySelectorAll('iframe').forEach(element => {
      const supported = [
        'www.youtube.com', 'player.vimeo.com', 'player.youku.com',
        'player.bilibili.com', 'www.tudou.com'
    ...
    window.addEventListener('scroll', () => {
      if (backToTop || readingProgressBar) {
        ...
        var contentVisibilityHeight =
            docHeight > winHeight ? docHeight - winHeight
                                  : document.body.scrollHeight - winHeight;
        var scrollPercent =
            Math.min(100 * window.scrollY / contentVisibilityHeight, 100);
        if (backToTop) {
          backToTop.classList.toggle('back-to-top-on',
    ...
    document.querySelectorAll('.tabs ul.nav-tabs .tab').forEach(element => {
      element.addEventListener('click', event => {
        ...
        if (!target.classList.contains('active')) {
          ...
          [...tActive.parentNode.children].forEach(
              element => { element.classList.remove('active'); });
          tActive.classList.add('active');
          // Trigger event
          tActive.dispatchEvent(new Event('tabs:click', {bubbles : true}));
        }
      });
    });
  ...
  registerCanIUseTag : function() {
    ...
    window.addEventListener('message', ({data}) => {
      if ((typeof data === 'string') && data.includes('ciu_embed')) {
        ...
        document.querySelector(`iframe[data-feature=${featureID}]`)
  ...
  registerLangSelect : function() {
    ...
    selects.forEach(sel => {
      ...
      sel.addEventListener('change', () => {
        ...
        document.querySelectorAll('.lang-select-label span')
  ...
  registerSidebarTOC : function() {
    ...
    function activateNavByIndex(target) {
      ...
      // Scrolling to center active TOC element if TOC content is taller then
      // viewport.
      window.anime({
        targets : tocElement,
        duration : 200,
        easing : 'linear',
        scrollTop : tocElement.scrollTop - (tocElement.offsetHeight / 2) +
    ...
    function createIntersectionObserver(marginTop) {
      ...
      let intersectionObserver =
          ...
          }, {rootMargin : marginTop + 'px 0px -100% 0px', threshold : 0});
      sections.forEach(
          element => { element && intersectionObserver.observe(element); });
    }
  ...
  isTablet : function() {
    return window.screen.width < 992 && window.screen.width > 767 &&
           this.hasMobileUA();
  },

  isMobile :
      function() { return window.screen.width < 767 && this.hasMobileUA(); },
  ...
  updateSidebarPosition : function() {
    ...
    if (!this.isDesktop() || CONFIG.scheme === 'Pisces' ||
    ...
    if (typeof display !== 'boolean') {
      ...
      display = CONFIG.sidebar.display === 'always' ||
